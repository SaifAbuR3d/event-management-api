...
public record SetOrganizerProfileCommand(string? About,
    ...
    : IRequestHandler<SetOrganizerProfileCommand, Unit>
{
    public async Task<Unit> Handle(SetOrganizerProfileCommand request, CancellationToken cancellationToken)
    {
        ...
        if (string.IsNullOrWhiteSpace(request.About) && string.IsNullOrWhiteSpace(request.Website)
        ...
        organizer.SetProfile(request.About, request.Website,
